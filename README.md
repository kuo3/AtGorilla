...
指定した問題の解答として、AtCoderに`./archive/<コンテスト名>/<プログラムID>.hs`の内容を提出します。  
...
githubからクローンして `stack install`でインストールしてください。  
環境変数`path`に配置したくない場合は、`stack build`でビルドして、
`atg`バイナリ(`stack new`でプロジェクトを作成した場合は、`atgs`バイナリ)を2.で作成するコンテスト用プロジェクトに
配置してください。
```
cd <作業フォルダ>
...
環境構築は、以下のページ様を参考にしました。
https://github.com/haskell-jp/atcoder-haskell-resources/blob/master/spec.md  
（※直接関係ない話ですが、  
　自分の環境では、GHC_ENVIRONMENT環境変数を設定した状態で、VSCODEを起動すると、
　stack newで作ったその他のプロジェクトでVSCODE-HASKELL(haskell-language-server)が正しく動かなくなってしまいました。
　暫定回避策として、`cabal install`でパッケージをインストールする時に`--package-env <envファイル>`の設定を削除しました。
  これでデフォルトで両方のプロジェクトでVSCODE-HASKELLが動くようになりました。
  (他に不都合でるかもしれませんが詳しくないのでよくわかってません。ghcupの環境は当面AtCoder用でのみ使う予定です。)
　　また、こちらも直接関係ない話なんですが、  
　`.cabal/bin`と`.ghcup/bin`のPATHへの設定を、シェルが読むこむよう(.zshenvとか)に設定しても、VSCODEを起動したときに、
　VSCODE-HASKELL(haskell-language-server)が読んでくれなかったので、注意です。
　自分は、`~/.profile`に設定しています。(Manjaro Gnome)）
...
(また、`stack new`でプロジェクトを作成した場合は、`atgs`バイナリを使用します。
 こちらのバイナリはそのうち削除予定です。)
...
- パースエラー時のエラーメッセージが文字化けしている。
  
## TODO
  
- サーバの負荷低減のために、テストデータの取得タイミングをいい感じにすくなくする。
- インタラクティブモードを追加したい。
