...
module AtGLib ( Mode(..), cc, pwc, login, logout, inifile, test, testw, submit
              , status, archive, extract ) where
...
cc mode arg = do
  ...
  if pe then do
    ...
    if pisl then do
      ...
      createFileLink (archiveFolder ++ "/" ++ arg) currentLink else E.throw
      $ newApplicationException "INFO-cc-1"
      "currentパスにリンク以外のファイルがあります。currentリンクは作成しません。"
    else createFileLink (archiveFolder ++ "/" ++ arg) currentLink
  pe <- doesPathExist skeletonFile
  unless pe $ writeFile skeletonFile ""
  pe <- doesPathExist $ archiveFolder ++ "/" ++ arg
  unless pe $ do
    createDirectoryIfMissing True $ archiveFolder ++ "/" ++ arg
    eSkeleton <- E.try getSkeletonContent
    case eSkeleton of
      Left exception@(ApplicationException code msg stack)
        -> E.throw $ newSystemExceptionStack "ERROR-cc-1"
        "内部エラーが発生しました。特定不能です。" exception
      Right skeleton -> mapM_
        (\i -> writeFile (archiveFolder ++ "/" ++ arg ++ "/"
                          ++ [ chr (ord 'a' + i) ] ++ ".hs") skeleton)
        [ 0 .. 5 ]
...
pwc = do
  ...
  case eContestName of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-pc-1"
      "コンテスト情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
    Right contestName -> do
      putStrLn
        $ "現在作業中のコンテストは " ++ contestName ++ " です。"
  return ()
...
login = do
  ...
  case result of
    Left (ApplicationException code msg stackmsg) -> do
      newLogin
    Right loginInfo -> E.throw $ newApplicationException "INFO-login-1"
...
logout = do
  ...
  let fUrlEncodedBody
        = urlEncodedBody [ ( "csrf_token", fromString csrfToken ) ]
  ( response, cookieJar )
    <- postAccess stringUrl fUrlEncodedBody cookieJar manager
  let body = responseBody response
  case parse logoutParser
    "レスポンスボディのパースに失敗しました。"
    (UTF8ByteString body) of
      Left msg -> E.throw $ newSystemException "ERROR-logout-1" $ show msg
      Right success -> if success then do
        writeFile loginInfoFile ""
        putStrLn "ログアウト処理が完了しました。"
        return () else do
        writeFile loginInfoFile ""
        E.throw $ newSystemException "ERROR-logout-2"
          "ログアウト処理に失敗しました。ログイン情報を破棄しました。"

inifile :: String -> IO ()
inifile arg = do
  eSkeleton <- E.try getSkeletonContent
  let skeleton = case eSkeleton of
        Left exception@(ApplicationException code msg stack) -> ""
        Right skeleton -> skeleton
  eContestName <- E.try getContestName
  contestName <- case eContestName of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-initfile-1"
      "コンテスト情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
    Right contestName -> return contestName
  writeFile (targetSrc contestName arg) skeleton
  return ()
...
test arg = do
  ...
  case eMode of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-test-1"
      "モード情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
    Right mode -> do
      eContestName <- E.try getContestName
      case eContestName of
        Left exception@(ApplicationException code msg stack)
          -> E.throw $ newApplicationExceptionStack "INFO-test-2"
          "コンテスト情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
        Right contestName -> do
          let watchTarget = if mode == GhcMode
                then targetSrc contestName arg else "./app/Main.hs"
...
testw arg = do
  ...
  case eMode of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-testw-1"
      "モード情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
    Right mode -> do
      eContestName <- E.try getContestName
      case eContestName of
        Left exception@(ApplicationException code msg stack)
          -> E.throw $ newApplicationExceptionStack "INFO-testw-2"
          "コンテスト情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
        Right contestName -> do
          let watchTarget = if mode == GhcMode
                then targetSrc contestName arg else "./app/Main.hs"
          ...
          wd <- SI.addWatch inotify [ SI.CloseWrite ] (fromString watchTarget)
            (\e -> do
  ...
  where
    ...
    testwp arg wd inotify watchTarget = do
      putStrLn $ watchTarget
        ++ "ファイルの変更確認中。変更確認されれば" ++ arg
        ++ "問題サンプルのテストを行います。\n"
    ...
    Right mode -> do
      ...
      case eContestName of
        Left exception@(ApplicationException code msg stack)
          ...
          "コンテスト情報を取得できなかったため、実行できません。ccオプションを実行してください。" exception
        Right contestName -> do
          submitFile <- if mode == GhcMode then BS.readFile
            $ targetSrc contestName arg else BS.readFile mainhsFile
          manager <- newManager tlsManagerSettings
          ( contestName, csrfToken, cookieJar ) <- preLoginWithCookie manager
          let url = "https://atcoder.jp/contests/" ++ contestName ++ "/submit"
          let fUrlEncodedBody = urlEncodedBody
...
status = do
  ...
  let body = responseBody response
  case parse submissionsMeParser
    "レスポンスボディのパースに失敗しました。"
    (UTF8ByteString body) of
      Left errmsg
      ...
      Right submissionsMe -> do
        let p = " | "
        putStrLn $ "       Submit time      " ++ p ++ "   Question name    "
          ++ p ++ "      Language      " ++ p ++ " Score" ++ p ++ "Result" ++ p
          ++ " Exec time"
        let submissionsMeStr = foldl
              (\acc x
               -> (submitTime x ++ p ++ take 20
                   (questionName x ++ replicate 20 ' ') ++ p ++ take 20
                   (language x ++ replicate 20 ' ') ++ p
                   ++ (replicate (6 - length (score x)) ' ' ++ score x) ++ p
                   ++ (replicate (6 - length (result x)) ' ' ++ result x) ++ p
...
archive arg = do
  ...
  case tGetContestName of
    Left exception@(ApplicationException code msg stack) -> putStrLn
      "コンテスト名の取得に失敗しました。ccオプションを実行してください。"
    Right contestName -> do
      let fromFile = mainhsFile
      let toFile = targetSrc contestName arg
      let bkFile = targetSrcBK contestName arg
...
extract arg = do
  ...
  case tGetContestName of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-archive-1"
      "コンテスト名の取得に失敗しました。ccオプションを実行してください。" exception
    Right contestName -> do
      let fromFile = targetSrc contestName arg
  ...
  case parse searchTokenParser
    ...
    (UTF8ByteString body1) of
      ...
      Right csrfToken -> do
        ...
        case parse (loginParser $ decode $ BS.unpack username)
          "レスポンスボディのパースに失敗しました。"
          (UTF8ByteString body2) of
            ...
            Right success -> if success then do
              ...
              putStr "ログイン処理に成功しました。" else E.throw
              $ newApplicationException "INFO-newLogin-1"
              "ログインできませんでした。ユーザ名とパスワードを見直してください。"

getLoginInfo :: IO String
getLoginInfo = do
  fe <- doesFileExist loginInfoFile
  unless fe $ E.throw $ newApplicationException "INFO-getLoginInfo-1"
    "ログイン情報を記録したファイルが見つかりません。"
  loginInfo <- readFile loginInfoFile
  when (loginInfo == "") $ E.throw
    $ newApplicationException "INFO-getLoginInfo-2"
    "ログイン情報を記録したファイルの内容が空です。"
  return loginInfo

testP :: String -> Mode -> String -> IO ()
testP arg mode contestName = do
  let buildCommandStr = if mode == GhcMode then "ghc -o "
        ++ targetBin contestName arg ++ " -O0 " ++ targetSrc contestName arg
  ...
  case resultPreLogin of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-testP-1"
      "事前ログインに失敗しました。" exception
    Right ( contestName, csrfToken, cookieJar ) -> do
      let url = "https://atcoder.jp/contests/" ++ contestName ++ "/tasks/"
            ++ contestName ++ "_" ++ arg
      let fSetQueryString
            = setQueryString [ ( "csrf_token", Just $ fromString csrfToken ) ]
      ( response, cookieJar ) <- getAccess url fSetQueryString cookieJar manager
      let body = responseBody response
      case parse searchSampleParser
          ...
          Right samples -> do
            ...
            case exitCode of
              ExitFailure exitCode
                -> E.throw $ newApplicationException "INFO-testP-2"
              ...
              ExitSuccess -> do
                ...
                forM_ [ 0 .. length samples - 1 ] $ \i -> do
                  ...
                  let iname = testFolder ++ "/" ++ arg ++ "_" ++ show no ++ "_"
                        ++ "i"
                  let oname = testFolder ++ "/" ++ arg ++ "_" ++ show no ++ "_"
                        ++ "o"
                  let outFileName = testFolder ++ "/out.txt"
                  writeFile iname $ exCR inData
                  writeFile oname $ exCR outData
                  let runCommandStr = if mode == GhcMode then "./"
                        ++ targetBin contestName arg ++ " >" ++ outFileName
                        ++ " < " ++ iname
                  ...
                  if exCR act == exCR outData then putStrLn
                    $ "入力例" ++ show no ++ "---------> OK" else do
...
preLoginWithCookie
  :: Manager -> IO ( StringContest, StringCsrfToken, CookieJar )
preLoginWithCookie manager = do
  tGetContestName <- E.try getContestName
  case tGetContestName of
    Left exception@(ApplicationException code msg stack)
      -> E.throw $ newApplicationExceptionStack "INFO-preLogin-1"
      "コンテスト名取得に失敗しました。" exception
    Right contestName -> do
...
postAccess :: StringUrl -> FUrlEncodedBody
  -> CookieJar -> Manager -> IO ( Response BSL.ByteString, CookieJar )
...
getAccess :: StringUrl -> FSetQueryString
  -> CookieJar -> Manager -> IO ( Response BSL.ByteString, CookieJar )
...
getContestName = do
  ...
  if not feContestFile then E.throw
    $ newApplicationException "INFO-getContest-1"
    "コンテスト情報を記録したファイルが作成されていないため、実行できません。ccオプションを実行してください。" else do
    contestName <- readFile contestFile
    if contestName == "" then E.throw
      $ newApplicationException "INFO-getContest-2"
      "コンテスト情報を記録したファイルが空のため、実行できません。ccオプションを実行してください。" else do
      return contestName

getSkeletonContent :: IO String
getSkeletonContent = do
  feSkeletonContent <- doesFileExist skeletonFile
  if not feSkeletonContent then E.throw
    $ newApplicationException "INFO-getSkeletonContent-1"
    "スケルトンファイルが見つかりません。" else do
    skeletonContent <- readFile skeletonFile
    return skeletonContent

getMode :: IO Mode
getMode = do
  feModeFile <- doesFileExist modeFile
  if not feModeFile then E.throw $ newApplicationException "INFO-getMode-1"
    "モード情報を記録したファイルが作成されていないため、実行できません。ccオプションを実行してください。" else do
    modeStr <- readFile modeFile
    if
      | modeStr == "GhcMode" -> return GhcMode
      | modeStr == "StackMode" -> return StackMode
      | otherwise -> E.throw $ newApplicationException "INFO-getMode-2"
        "モード情報を記録したファイルの内容が無効なため、実行できません。ccオプションを実行してください。"

getMainhsBS :: IO BS.ByteString
getMainhsBS = do
  feMainhs <- doesFileExist mainhsFile
  if not feMainhs then E.throw $ newApplicationException "INFO-getMainhsBS-1"
    "app/Main.hsファイルが存在しません。" else do
    mainhs <- BS.readFile mainhsFile
    if mainhs == "" then E.throw $ newApplicationException "INFO-getMainhsBS-2"
      "app/Main.hsファイルが空です。" else do
      return mainhs

moveFile :: String -> String -> String -> IO ()
moveFile fromFile toFile bkFile = do
  feFrom <- doesFileExist fromFile
  if not feFrom then E.throw $ newApplicationException "INFO-moveFile-1"
    "fromファイルが存在しないため、実行できません。プロジェクトの構成を見直してください。" else do
