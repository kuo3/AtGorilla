...
pwc = do
  ...
  case eContestName of
    Left exception@(ApplicationException code msg stack) -> E.throw
...
logout = do
  ...
  ( response
    , cookieJar) <- postAccess stringUrl fUrlEncodedBody cookieJar manager
    ...
    Right mode -> do
      ...
      case eContestName of
        ...
        Right contestName -> do
          ...
          ( response
            , cookieJar) <- postAccess url fUrlEncodedBody cookieJar manager
          putStrLn "提出しました。"
          return ()
  ...
  case parse
    ...
    Right csrfToken -> do
      ...
      let fUrlEncodedBody = urlEncodedBody
            ...
            , ("password", password)]
      ( response
        , cookieJar) <- postAccess stringUrl fUrlEncodedBody cookieJar1 manager
...
testP arg mode contestName = do
  ...
  case resultPreLogin of
    ...
    Right (contestName, csrfToken, cookieJar)            -> do
      let url = "https://atcoder.jp/contests/"
            ++ contestName
            ++ "/tasks/"
            ++ contestName
            ++ "_"
