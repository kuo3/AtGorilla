...
searchSampleParser = do
  ...
  where
    ...
    sampleParser = do
      ...
      skipMany $ space <|> newline
      o <- many $ noneOf "<"
      manyTill anyChar ((try (string "<h3>入力例 ") >> many1 digit) <|> try
                        (eof >> return ""))
      return ( i, o )

data SubmissionsMe
  = SubmissionsMe { submitTime :: String, questionName :: String, userName
      :: String, language :: String, score :: String, codeLength
      :: String, result :: String, execTime :: String, memoryUsage :: String }
  deriving ( Show )
...
submissionsMeParser = do
  ...
  where
    ...
    skipMany $ space <|> newline-}
      ( result, execTime, memoryUsage ) <- try td3Parser
        <|> (do
               result <- tdParser
               execTime <- tdParser
               memoryUsage <- tdParser
      ...
      manyTill anyChar (try (string "</tr>"))
      skipMany $ space <|> newline
      return SubmissionsMe { submitTime
        = submitTime, questionName = questionName, userName = userName, language
        = language, score = score, codeLength = codeLength, result
        = result, execTime = execTime, memoryUsage = memoryUsage }
    ...
    tdContentParser = try (many1 $ noneOf "<")
      <|> (char '<' >> manyTill anyChar (try (char '>')) >> tdContentParser)

    td3ContentParser = try (many1 $ noneOf "<")
      <|> (try (string "<td colspan='3' class='text-center'><") >> manyTill
           anyChar (try (char '>')) >> td3ContentParser)
